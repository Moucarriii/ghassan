...
if Menu == "Home":
  ...
  <header class="bg-gold sans-seSans Serif">
    <div class="mw9 center pa4 pt5-ns ph7-l">
      ...
      <h5 class="f6 ttu tracked black-80">By Ghassan Shammas</h5>
      </div>
      </p>
      </div>
...
#Top 10 countries which recored the highest dynamics content rates across the years
  if Menu=="Dashboard":st.header("Top 10 Countries with the highest dynamics content Rate")

  if Menu=="Dashboard":Year = st.slider('Select The year Please', 2008, 2021, 2009)

  if Menu=="Dashboard":datayear = df[df["year"]==Year]

  if Menu=="Dashboard":f, ax = plt.subplots(1,1, figsize=(10,8))
  if Menu=="Dashboard":data_country_total = datayear.groupby(by=['country']).agg({'country name': ['sum']})
  if Menu=="Dashboard":data_country_total.columns = ['type']
  if Menu=="Dashboard":data_country_total.reset_index(inplace=True)
  if Menu=="Dashboard":data_country_total = data_country_total.sort_values(by=['type'], ascending=False).head(10)

  if Menu=="Dashboard":fig = px.bar(data_country_total, x='type', y='country',orientation='h')

  if Menu=="Dashboard":plt.title('Top 10 Countries with the highest dynamics content Rate')
